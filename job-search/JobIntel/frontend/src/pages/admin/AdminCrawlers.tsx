...
// Supported countries for scraping (matches backend country mapping)
        ...
        if (currentSessionId) {
          ...
          if (currentLog) {
            ...
            if (currentLog.status === 'completed') {
              setSuccessMessage(`✅ Scraping completed! Found ${currentLog.totalJobsFound} jobs (${currentLog.indianJobsFound} from ${selectedCountry}) - ${currentLog.newJobsAdded} new added, ${currentLog.jobsUpdated} updated`);
              setMongoMessage(`✨ MongoDB updated: ${currentLog.newJobsAdded} new documents added to 'jobs' collection (${currentLog.indianJobsAdded} from ${selectedCountry})`);
  ...
  async function startScraping() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMongoMessage(`⏳ Scraping in progress... Connecting to OpenWeb Ninja API for ${selectedCountry} jobs`);
    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-3xl font-bold text-foreground">
          {SUPPORTED_COUNTRIES.find(c => c.value === selectedCountry)?.flag || '🌍'} Web Crawlers & Scraping
        </h2>
        <p className="text-sm text-muted-foreground mt-1">
          Real-time job scraping from OpenWeb Ninja API. Currently targeting: <strong>{selectedCountry}</strong> jobs. Monitor progress per bucket.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card className="p-4 bg-gradient-to-br from-green-50 to-green-100 dark:from-green-950 dark:to-green-900">
            <p className="text-xs text-green-600 dark:text-green-300 font-medium uppercase mb-1">
              {selectedCountry} Jobs {SUPPORTED_COUNTRIES.find(c => c.value === selectedCountry)?.flag || '🌍'}
            </p>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          {/* Country Selection */}
          <div>
            <label className="text-sm font-medium mb-2 block">🌍 Select Target Country</label>
            <select
            ...
            <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
              Choose the country to scrape jobs from. Backend will automatically map to ISO codes for API compatibility.
            </p>
          </div>
          ...
          <div className="flex items-center space-x-3 p-3 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-lg">
            ...
            <label htmlFor="filterIndian" className="text-sm font-medium cursor-pointer">
              🇮🇳 Filter for {selectedCountry} Jobs Only (Recommended)
            </label>
          </div>
          ...
          <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded p-4 text-sm text-blue-900 dark:text-blue-100 space-y-2">
            <p>
              <strong>🔍 Filtering:</strong> {selectedCountry} jobs only {filterIndianOnly ? '✅ ENABLED' : '❌ DISABLED'}
            </p>
            <p>
              <strong>📍 Location:</strong> {filterIndianOnly ? `${SUPPORTED_COUNTRIES.find(c => c.value === selectedCountry)?.flag || '🌍'} ${selectedCountry}` : '🌍 Global'} {filterIndianOnly ? `(${selectedCountry === 'India' ? 'Mumbai, Delhi, Bangalore, Hyderabad, Pune, Gurgaon, etc.' : 'Major cities in selected country'})` : ''}
            </p>
            <p>
              <strong>🌐 Country:</strong> {selectedCountry} (ISO code: {selectedCountry === 'India' ? 'in' : selectedCountry === 'United States' ? 'us' : selectedCountry === 'United Kingdom' ? 'uk' : selectedCountry === 'Canada' ? 'ca' : selectedCountry === 'Australia' ? 'au' : selectedCountry === 'Germany' ? 'de' : selectedCountry === 'France' ? 'fr' : selectedCountry === 'Netherlands' ? 'nl' : selectedCountry === 'Singapore' ? 'sg' : selectedCountry === 'Spain' ? 'es' : 'unknown'})
            </p>
            <p>
              <strong>📄 Multi-Page Scraping:</strong> ✅ 10 pages per bucket (500+ jobs per category)
            </p>
            <p>
              <strong>🏙️ Cities Covered:</strong> {selectedCountry === 'India' ? 'Bangalore, Delhi, Mumbai, Hyderabad, Pune, Gurgaon, Kolkata, Chennai, Jaipur, Indore, Chandigarh, Kochi, Ahmedabad, Surat, Nagpur, and 15+ more' : 'Major metropolitan areas and business hubs in selected country'}
            </p>
            <p>
              <strong>🏢 Companies Detected:</strong> {selectedCountry === 'India' ? 'TCS, Infosys, Wipro, HCL, Cognizant, Flipkart, Amazon India, Zomato, PayPal, Microsoft India' : 'Major companies and tech firms in selected country'}
            </p>
            <p>
